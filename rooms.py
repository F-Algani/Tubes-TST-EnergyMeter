...
class Room(BaseModel):	#ubah isinya
	room_id: int
	room_name: str
...
async def add_room(room: Room):
	...
	for room_item in data['rooms']:
		if room_item['room_id'] == room_dict['room_id']:
			room_found = True
			return "Room ID "+str(room_dict['room_id'])+" exists." #ubah jadi fungsi nambah store & bukunya
...
async def update_room(room: Room):
	...
	for room_idx, room_item in enumerate(data['rooms']):
		if room_item['room_id'] == room_dict['room_id']:
			...
			with open(json_filename,"w") as write_file:
				json.dump(data, write_file)
			return "Room updated"	#ubah jadi fungsi ganti bukunya
